...
class Controleur():
    ...
    def genereAssets(self,participants):
        ...
        for i in j:
            ...
            while pte==None:
                qrt=random.choice(self.modele.ville.quartiers)
                if qrt.pates: # assure que c'est 
                    pte=random.choice(qrt.pates)
            
            otox=pte.cadreTx1
            otoy=pte.cadreTy1
            
            print("PTE",pte.cadreTx1,pte.cadreTy1,pte.cadreTx2,pte.cadreTy2,otox,otoy)
            
            """ premier essaie (pas bon)
            if self.modele.ville.quartiers[0].pates:
                v=self.modele.ville.quartiers[0].pates[0]
                print("CADRE COIN !",v.cadreTx2,v.cadreTy2)
                otox=v.cadreTx2+2
                otoy=v.cadreTx2+2
            
            elif self.modele.ville.quartiers[0].parcs:
                v=self.modele.ville.quartiers[0].parcs[0]
                print("PARC COIN !",v.cadreTx2,v.cadreTy2)
                otox=v.cadreTx2+2
                otoy=v.cadreTx2+2
            """ # fin premier essai
            
            #nouveau.position=[lex,pos[1],1] # replace nouveau (qui contient le dernier objet cree) au x (de lex), le y actuel, le z ==1
            nouveau.position=[otox,otoy,0] # replace nouveau (qui contient le dernier objet cree) au x (de lex), le y actuel, le z ==1
            ...
            participants[i].auto.x=otox # on modifie les proprietes de position de l'Auto dans le modele ici, le x
            participants[i].auto.y=otoy # ici le y
            participants[i].auto.z=0 # ici le y
            ...
            if i==self.monnom: # si le participant pour lequel on vient de creer un objet 3D est 'moi' c'est a dire mon char pas celui des autres participants
                ...
                cam.position=[otox,otoy,1.5]
                cam.setParent(nouveau, False,True) # alors on parente la camera a cette auto pour la suivre dans ses deplacements
                camOis=objets["camVoldoiseau"] # notre camera
                camOis.position=[otox,otoy-20,2]
                camOis.setParent(nouveau, False,True) # alors on parente la camera a cette auto pour la suivre dans ses deplacements
                camEsp=objets["camEspace"] # notre camera
                camEsp.position=[otox,otoy,400]
                camEsp.setParent(nouveau, False,True) # alors on parente la camera a cette auto pour la suivre dans ses deplacements
                print("CAMS ",cam.position,camOis.position,camEsp.position,nouveau.position)
